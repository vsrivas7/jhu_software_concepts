...
def _extract_decision_parts(decision_parts: list) -> tuple:
    """Extract season, applicant_status, and gpa from decision cell parts."""
    season = ""
    applicant_status = ""
    gpa = ""
    ...
    return season, applicant_status, gpa
...
class GradCafeScraper:
    ...
    def scrape_page(self, page_num: int = 1, params: Dict = None) -> List[Dict]:
        ...
        try:
            ...
            for row in rows:
                result = self._parse_row(row, page_num)
                if result:
    ...
    @staticmethod
    def _parse_row(row, page_num: int) -> Dict:
        ...
        try:
            ...
            decision_parts = (
                cells[3].get_text(separator="|", strip=True).split("|")
                if cells[3] else []
            )
            decision = decision_parts[0].strip() if decision_parts else ""

            season, applicant_status, gpa = _extract_decision_parts(decision_parts)

            see_more_link = row.find("a", href=re.compile(r"/result/\d+"))
            result_id = see_more_link["href"].split("/")[-1] if see_more_link else ""
...
def scrape_data() -> List[Dict]:
    ...
    Returns a small dataset for the pull-data endpoint.
    """
    scraper = GradCafeScraper()
    return scraper.scrape_dataset(num_pages=1)
